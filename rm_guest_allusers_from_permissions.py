...
class TableauPermissionCleaner():
        ...
        for w in workbooks:
            ...
            for p in w.permissions:
                if p.grantee.id == group_id and p.grantee.tag_name == 'group':
                    self.lgr.info(f'Remove {str(p.capabilities)} permissions from wb: "{w.name}" , project: "{w.project_name})"')
                    if not self.noop:
    ...
    def _clear_site_from_user(self, ignore_tag, user_id):
        ...
        for d in datasources:
            ...
            for p in d.permissions:
                if p.grantee.id == user_id and p.grantee.tag_name == 'user':
                    self.lgr.info(f'Remove {str(p.capabilities)} permissions from datasource: "{d.name}" , project: "{d.project_name})"')
                    if not self.noop:
                        self.tsc_server.datasources.delete_permission(w, p)
...
def main():
    ...
    tbl_cleaner = TableauPermissionCleaner(url=cred['server'], username=cred['username'], password=cred['password'], noop=argz.get('--noop'))
